...
!-----------------------------------------------------------------------
! Subroutine name: Langevin
! Subroutine description: 1D Lagrangian macromixing (modello di Thomson 
! in turbolenza omogenea, rilascio puntuale e istantaneo). Year: 2006.
!-----------------------------------------------------------------------
      PROGRAM LANGEVIN
!     dati
...
!     stepzmax=40, steptmax=1000
	  ...
	  REAL Zmat(10,1000), depsmat(10,1000)
!     osservo 10 particelle, stetmax=1000
	  REAL Wvet(1000)
!     steptmax=1000
...
!	  WRITE (*,*) 'Inserisci la varianza della velocità verticale (sigma2w, valore di riferimento: 0.01(m^2/s^2), valore max: 1.00(m^2/s^2): '
...
!     fornisce le concentrazioni iniziali
      REAL C(40,1000)
	  real n
	  real stepzmax
	  stepzmax=40
...
!     fornisce la pdf gaussiana data la media "a" e la varianza "sigma2"
...
!     estrae una variabile con pdf gaussiana
	  REAL m
